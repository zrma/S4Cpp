...
namespace S4Framework
{
	...
	public:
		...
		void DoSync(R(T::*memfunc)(Args...), Args... args)
		{
			...
			auto task = [=](Args... args) { return (static_cast<T*>(this)->*memfunc) (std::forward<Args>(args)...); };
			mPool.PostJob(mWrapper.wrap(task));
		}
	...
	void DoSyncAfter(uint32_t after, T instance, F memfunc, Args&&... args)
	{
		...
		auto timer = std::make_shared<boost::asio::steady_timer>(std::static_pointer_cast<SyncExecutable>(instance)->mPool.GetDispatcher());
		timer->expires_from_now(std::chrono::milliseconds(after));

		auto owner = std::static_pointer_cast<SyncExecutable>(instance);
